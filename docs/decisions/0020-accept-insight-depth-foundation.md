...
WideNote's early insight loop was intentionally conservative: deterministic
Memory-first projections created source-linked summary, count, trend, and
source-mix insights. Product review rejected that as the primary direction.
Insight should mean a heavier, model-backed reflection over recent state, with
evidence, uncertainty, and review gates. The product therefore needs a
dedicated Insights surface and a public contract for richer behavior, contrast,
turning-point, or evidence-based insights generated by Agent Packs.
...
3. Add a Home-owned mobile Insights page at `/insights` and detail page at
   ...
   claims, metrics, evidence, counter-evidence, and source refs. ADR-0022 later
   removed the archive/restore lifecycle action from the mobile Insights
   surface.
4. Keep deterministic card generation for cards only. The default capture loop
   and `packages/dart/cards` must not emit summary/count/trend/source-mix
   lightweight insights as a fallback.
5. Route automatic insight generation to a future dedicated official Pack,
...
users a place to inspect evidence. Deferring deep
...
[ADR-0022](./0022-drop-insight-archive-actions.md) supersedes only the mobile
archive/restore lifecycle action portion of this ADR. The source-linked Insights
page, detail page, schema foundation, deep insight guardrails, and dedicated
future Pack direction remain accepted.
